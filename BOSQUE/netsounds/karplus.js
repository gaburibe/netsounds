...
if (fresco_detune==0) {
	fresco_detune=2
	console.log("entering fresco", "detune", fresco_detune)
	
}
else{
	console.log("existed fresco", "detune", fresco_detune)
}

if (fresco_duration==0) {
	fresco_duration=2
	console.log("entering fresco", "fresco_duration", fresco_duration)
	
}
else{
	console.log("existed fresco", "fresco_duration", fresco_duration)
}
...
/*
 * look for the phrase
 * UI2karplus32 to see where the UI sends messages to karplus32
...
function karplus32Module(global, foreign, buffer) {
	...
	function instanceResetUserInterface(dsp) { //FRESCO
		dsp = dsp | 0;
		HEAPF[dsp + 481696 >> 2] = 0.2;//level volume //
		HEAPF[dsp + 481700 >> 2] = 18.; //polyphony
		HEAPF[dsp + 481704 >> 2] = 0.000000001; //attenuation
		HEAPF[dsp + 481708 >> 2] = 0.2 ;//level volume //
		HEAPF[dsp + 481712 >> 2] = 0.;
		HEAPF[dsp + 481716 >> 2] = 180.; // excitation //
		HEAPF[dsp + 481720 >> 2] = fresco_duration;//duration  //
		HEAPF[dsp + 481724 >> 2] = fresco_detune; //detune
...
function getPathTablekarplus32() {
	...
	pathTable["/karplus32/resonator_x32/attenuation"] = 0;
...
function changeBufferSize(buffer_size)
{
    ...
    startNewkarplus32(new_buffer_size);
}
...
function getDevice(device) 
{
    ...
    audio_input.connect(karplus32.getProcessor());
}

function startkarplus32Aux(buffer_size) 
{
    console.log("startkarplus32 %d", buffer_size);
    if (karplus32) {
        if (audio_input) {
            audio_input.disconnect(karplus32.getProcessor());
        }
        _f4u$t.hard_delete(faustsvg);
        karplus32.stop();
        karplus32.destroy();
    }
    karplus32 = faust.karplus32(audio_context, buffer_size);
    if (karplus32.getNumInputs() > 0) {
        activateAudioInput();
    }
    karplus32.start();
    console.log(karplus32.json());
    console.log(karplus32.controls());
    ...
    var handler = _f4u$t.main(karplus32.json(), faustsvg, karplus32.setParamValue);
    karplus32.setHandler(handler);
}

function startkarplus32() 
{
    startkarplus32Aux(1024);
}

function startNewkarplus32(buffer_size) 
{
    startkarplus32Aux(buffer_size);
}
